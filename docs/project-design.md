...
- 状态迁移逻辑应尽量纯函数化或最小副作用化，便于单元测试
- 每个状态步骤应能被独立构造输入并验证输出，避免把测试强耦合到完整运行环境
...
同时建议提供可替换的 fake/stub 实现，用于 TDD 和离线测试，避免测试直接依赖真实模型接口。
...
- 业务逻辑与 IO 操作尽量分离
- Prompt 组装逻辑应可独立测试
- 状态机每一步应可注入假数据和假模型客户端
- 核心流程应支持在不访问真实外部模型的情况下完成测试
